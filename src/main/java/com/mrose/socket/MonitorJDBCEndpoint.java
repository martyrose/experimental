 ...
 */
public class MonitorJDBCEndpoint {
    ...
    public static void main(String[] args) {
        if( args.length != 3 ) {
            ...
            log(java.util.Arrays.asList(args).toString());
            return;
        }
        String url = args[0];
        ...
        try {
            Class clazz = MonitorPersistentJDBCEndpoint.class.getClassLoader().loadClass(DRIVER);
        ...
        while(true) {
            ...
            boolean success = doCreateEndpoint(url, username, password, s1, s2, s3);
            long end = System.currentTimeMillis();

            if(success) {
                System.out.println("S|" + sdf.format(new Date()) + "|" + (s2.value-s1.value) + "|" + (s3.value-s2.value));
            } else {
                System.out.println("F|" + sdf.format(new Date()) + "|" + (s2.value-s1.value) + "|" + (s3.value-s2.value));
            }
    ...
    private static boolean doCreateEndpoint(String url, String username, String password, MutableLong s1, MutableLong s2, MutableLong s3) {
