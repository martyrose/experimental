 ...
 */
public class MonitorURLEndpoint {
    ...
    public static void main(String[] args) {
        ...
        while(true) {
            ...
            boolean success = doCreateEndpoint(url, s1, s2, size);
            long end = System.currentTimeMillis();

            if(success) {
                System.out.println("S|" + sdf.format(new Date()) + "|" + (s2.value-s1.value) + "|" + size.value);
            } else {
                System.out.println("F|" + sdf.format(new Date()) + "|" + (s2.value-s1.value) + "|" + size.value);
            }
    ...
    private static boolean doCreateEndpoint(String url, MutableLong s1, MutableLong s2, MutableLong size) {
        InputStream is = null;
        try {
            s1.value = System.currentTimeMillis();
            URL u = new URL(url);
            is = u.openStream();
            size.value = (long)is.available();
    ...
    public static final void close(Closeable s) {
        if (s != null) {
            try {
                s.close();
            } catch (IOException e) {
